...
class battery:
    ...
    def __init__(self, capacity=14, dischargeable = False, importable = False, charge_rate=5, 
                 discharge_rate=5, round_trip_efficiency=0.9, solar_present=True):
        """
        Initialize battery specs.

        `capacity` is the total current energy capacity of the battery in kWh        

        `dischargeable` represents if the battery can send it's power back to the optimizer for use in another
    ...
    def get_discharge_potential(self):        
        """
        returns the maximum available energy output from the battery for some time interval, allowing 
        for discharge losses.
        
        inputs:
            avl_energy (float): total current energy present in the battery in kWh.
            discharge_loss (float): the percentage loss in energy due to discharging efficiency losses.
        
        return:
            discharge_max (float): the maximum available output from the battery over the given
                                time interval in kWh. 
        """
        avl_energy = self.avl_energy 
    ...
    def get_limits(self, solar, load):
        """
        Calculates the operation limits of the battery, given some inputs for the
        system solar outputs and household loads. It calculates the net engergy
        given some solar and house hold, and then depending on whether the battery
        is allowed to push HOMER into a state of grid export, will calculate a discharge 
        limit. Similarly, it also checks whether the battery is able to charge from the
        grid, and sets an appropriate charge limit. These limits take into account
        the maximum charge/discharge rate and the available/empty capacity by calling
        the .get_charge_potential() and the .get_discharge_potential() methods of
        the `battery` class. 
        
        The discharge limit is negative or zero, and the charge limit is positive or zero.
        Together these form a continuous range of allowable values for the battery.
        ...
        return:
            battery_limit (tuple): (discharge_limit, charge_limit)
                Interval endpoints bounding the allowable the battery operation settings where:
                discharge_limit (float): the max amount of energy the battery can discharge for 
                    the current system state in kWh.
                charge_limit (float): the max amount of ebergy the battery could receive to charge
                    within the current system state in kWh.
        """
