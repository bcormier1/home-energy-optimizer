...
class HomerEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None) -> Tuple[Any, Dict]:
        ...
        if self.episodes == -1:
            ...
            self.self._current_tick = self.start_tick
            self._end_tick = len(self.df) - 1
        else:
            self.start_tick = self._current_tick
            self.updated_action = None
            length = self.start_tick + (self.episodes * 12 * 24)
            if length > len(self.df):
                length = len(self.df) - self._current_tick - 1
                self._end_tick = self.start_tick + length
            else:
                self._end_tick = self.start_tick + length
